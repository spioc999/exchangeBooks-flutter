...
class LoggerInterceptor extends Interceptor {
  ...
  /// If you want use print, set logPrint to print
  ...
  Future onRequest(RequestOptions options) async {
    ...
    if (requestHeader) {
      if(compact){
        logPrint("Header : " + options.headers.toString());
      }else {
    ...
    //
    if(requestBody && compact){

      logPrint("QueryParam : " + options.queryParameters.toString());
      logPrint("BodyParam : " + options.data.toString());
    }
    //
    if (requestBody && options.method != 'GET' && !compact) {
  ...
  void _printRequestHeader(RequestOptions options) {
    ...
    final uri = options.baseUrl.length > 0 ? options?.uri : "";
...
//      key = '\"$key\"';
  ...
  static void debug(Object object) {
    String line = "$object";
    if (line.length > 5000) {
      debugPrint(line.substring(0,5000));
    } else {
      debugPrint(line);
    }
